    ...
    <table>
        ...
        <tr>
            ...
            <g:sortableColumn property="timestamp"
                              title="${message(code: 'message.timestamp.label', default: 'Timestamp')}"/>

            <g:sortableColumn property="fromUsername"
                              title="${message(code: 'message.fromUsername.label', default: 'From Username')}"/>

            <g:sortableColumn property="content" title="${message(code: 'message.content.label', default: 'Content')}"/>

            <g:sortableColumn property="read" title="${message(code: 'message.read.label', default: 'Read')}"/>
        ...
        <g:each in="${messageInstanceList}" status="i" var="messageInstance">
            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                ...
                <td><g:link action="show"
                            id="${messageInstance.id}">${fieldValue(bean: messageInstance, field: "timestamp")}</g:link></td>

                <td>${fieldValue(bean: messageInstance, field: "fromUsername")}</td>

                <td>${fieldValue(bean: messageInstance, field: "content")}</td>

                <td><g:formatBoolean boolean="${messageInstance.read}"/></td>
